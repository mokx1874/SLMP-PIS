...
class EllipticCurve:
    ...
    def add(self, P1, P2):
        ...
        if self.equalModp(x1, x2) and self.equalModp(y1,
                                                     -y2):  # in this case the points are negatives of each other in the EC group law
        ...
        if self.equalModp(x1, x2) and self.equalModp(y1, y2):
            u = self.reduceModp(3 * x1 * x1 + self.a) * self.inverseModp(
                2 * y1)  # geometrically this is the slope of the tangent line to the curve at the point ( implicitly differentiate y^2 = x^3 +ax +b)
        else:
            u = self.reduceModp(y1 - y2) * self.inverseModp(
                x1 - x2)  # geometrically its the slope of the line throught the points
        v = self.reduceModp(
            y1 - u * x1)  # geometrically this is the y-intercept of the line (either the tangent or through the points depending on the above if-else)
